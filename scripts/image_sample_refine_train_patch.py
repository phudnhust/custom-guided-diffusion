...
def main():
    ...
    refine_net = RefineINRWithTopK().to(dist_util.dev())
    ...
    for epoch in range(0, n_epoch):
        ...
        for hq_img_batch in hq_img_batches:
            ...
            for hq_img_path in hq_img_batch:
                ...
                residual = sample['residual'].squeeze(0)
                anchor = th.from_numpy(_codebooks[timestep][sample['retrieved_index']])
                neighbors = th.from_numpy(_codebooks[timestep][sample['top_sim_idx']])
                # print('residual.shape:',  residual.shape)  # torch.Size([3, 256, 256])
                # print('anchor.shape: ', anchor.shape)      # torch.Size(3, 256, 256)

                patch_z, patch_neighbors, rel_pos, timestep, patch_target = get_patch_data(anchor, neighbors, residual, timestep)
                patch_z_list.append(patch_z)
                rel_pos_list.append(rel_pos)
                t_list.append(th.tensor(timestep))
                patch_target_list.append(patch_target)
                neighbors_list.append(patch_neighbors)

            batch_patch_z = th.stack(patch_z_list, dim=0).to(dist_util.dev())
            batch_rel_pos = th.stack(rel_pos_list, dim=0).to(dist_util.dev())
            batch_t = th.stack(t_list, dim=0).to(dist_util.dev())
            batch_patch_target = th.stack(patch_target_list, dim=0).to(dist_util.dev())
            batch_neighbors = th.stack(neighbors_list, dim=0).to(dist_util.dev())

            # print('batch_patch_z.shape: ', batch_patch_z.shape)
            # print('batch_rel_pos.shape: ', batch_rel_pos.shape)
            # print('batch_t.shape: ', batch_t.shape)
            # print('batch_patch_target.shape: ', batch_patch_target.shape)

            loss = refine_net.train_individual_epoch(optimizer, batch_patch_z, batch_neighbors, batch_rel_pos, batch_t, batch_patch_target)
            epoch_loss_list.append(loss)
        
        if (epoch > 0 and epoch % 100 == 0) or epoch == n_epoch-1:
            refine_net.save_checkpoint(optimizer, epoch, path=repo_folder_path + f'patch_refine_net_{epoch}.pth')
